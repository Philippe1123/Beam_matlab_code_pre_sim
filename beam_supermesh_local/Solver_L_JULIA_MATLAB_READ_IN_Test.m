...
function [] = Solver_L_JULIA_MATLAB_READ_IN_Test(ProcID)
tic
ProcID=1;
...
str_interm=str_folder+"Interm9/Beam/";
...
%         str_elements=str_folder+"Mesh/Beam/p_refinement/";
        ...
        str_elements="/home/philippe/Desktop/Beam_matlab/Test_DifferentDiscretization/p_ref/Matlab/";
...
% seldof=unique(floor(DOF))+0.02;
% vec=ones(length(seldof),1)./length(seldof);
...
toc
tic
res_node=selectnode(Nodes,min(Nodes(:,2))-1e-6,min(Nodes(:,3))-1e-6,-inf,max(Nodes(:,2))+1e-6,min(Nodes(:,3))+1e-6,inf);

[posy,srt]=sort(res_node(:,2),'ascend');


res_node=res_node(srt,:);
...
for idx=1:nElem
 ...
 Nwres(jx)=sh_qs36(pt(1,jx),pt(2,jx))'*selectdof(DOF,Nod+0.02)*U(:,end); 
       String="Order 5";
           ...
           case 6
 Nwres(jx)=sh_qs49(pt(1,jx),pt(2,jx))'*selectdof(DOF,Nod+0.02)*U(:,end); 
       String="Order 6";
       
                                case 7
 Nwres(jx)=sh_qs64(pt(1,jx),pt(2,jx))'*selectdof(DOF,Nod+0.02)*U(:,end); 
       String="Order 7";
...
%  idx=find(pts(:,1)==2.5);
 ...
 pts_int=pts(idx,:);  
%  midelem=pts_int(find(pts_int(:,2)==0.5),end);
