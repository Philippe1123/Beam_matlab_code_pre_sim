...
ProcID=2
NumberOfGaussPoints=[9 25 49 61];
...
filesNodesMax=strcat(str_elements,"Nodes_L",fileMaxLevelI,".txt");
...
% % % %             end
% % % % end
QoINodes=selectnode(Nodes,-inf,max(Nodes(:,3))-1e-6,-inf,inf,max(Nodes(:,3))+1e-6,inf);
u=U(:,end);
 u_out=min(u)
...
res_node=selectnode(Nodes,max(Nodes(:,2))/2-1e-6,max(Nodes(:,3))-1e-6,-inf,max(Nodes(:,2))/2+1e-6,max(Nodes(:,3))+1e-6,inf);
