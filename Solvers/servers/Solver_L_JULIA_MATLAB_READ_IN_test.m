...
[posx,srt]=sort(Nodes(forceNodes,2),'ascend');
...
P = nodalvalues(DOF,seldof,[(cos(0:2*pi/(size(seldof,2)-1):2*pi)-1)/((size(seldof,2)-1))]);
...
res_node=selectnode(Nodes,max(Nodes(:,2))/2-1e-6,max(Nodes(:,3))/2-1e-6,-inf,max(Nodes(:,2))/2+1e-6,max(Nodes(:,3))/2+1e-6,inf);
