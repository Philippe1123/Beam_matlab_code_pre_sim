function [  ] = GenerateMeshes_beam_supermesh_global(  )
% close all
...
EltOpts.nl='elastoplastic';
 %figure;
foldername="beam_supermesh_global";
...
x=[];    
for id=0:numberoflevels_order    
if(id==0)    
x=gaussq(NumberOfGaussPoints(id+1));

else
x2=gaussq(NumberOfGaussPoints(id+1));   
...
PathGaussPoints=foldername+"/p_ref/Matlab/"+"GaussPoints_L_"+num2str(idOrder)+"_SUPERMESH_FULL_HIER.txt";
PathSortVec=foldername+"/p_ref/Matlab/"+"SortVec_L_"+num2str(idOrder)+".txt";

    elseif(higherOrder==true&&refinement==true)

PathGaussPoints=foldername+"/hp_ref/Matlab/"+"GaussPoints_L_"+num2str(idOrder)+"_SUPERMESH_FULL_HIER.txt";
